...
def init_page_layout():
    ...
    st.markdown("""
        <style>
            /* Custom styles for Streamlit layout */
            .block-container {
                padding-top: 0rem;
                padding-bottom: 0rem;
            }
...
def main(current_user=None):
    ...
    def listar_noticias():
        ...
        if noticias:
            render_noticia_list(noticias, noticia_service)
        else:
    ...
    def render_pagination_controls(total_pages):
        ...
        with cols[2]:
            if st.button("Ant.", disabled=st.session_state['page_number'] <= 1):
                ...
                st.rerun()
        with cols[1]:
            st.markdown(f"<div style='text-align: center; padding-top: 10px;'>{st.session_state['page_number']} - {total_pages}</div>", unsafe_allow_html=True)
        with cols[3]:
            if st.button("Próx.", disabled=st.session_state['page_number'] >= total_pages):
                st.session_state['page_number'] += 1
                st.rerun()

    def render_noticia_list(noticias, noticia_service):
        index = 0
        noticias_data = [noticia.model_dump() for noticia in noticias]

        for noticia in noticias_data:
            index += 1
            background_color = "#f0f0f0" if index % 2 == 1 else "#ffffff"

            col1, col2, col3 = st.columns([1, 8, 1])

            with col1:
                render_noticia_actions(noticia, noticia_service)

            with col2:
                render_noticia_details(noticia, noticia_service)

            with col3:
                render_noticia_metadata(noticia)

            st.markdown("---")

    def render_noticia_actions(noticia, noticia_service):
    ...
    def render_noticia_details(noticia, noticia_service):
    ...
    def render_noticia_metadata(noticia):
    ...
    def editar_noticia(noticia_id):
        ...
        if noticia:
            ...
            with st.form(key='edit_form'):
                ...
                valores = {campo: get_input_value(noticia, campo) for campo in campos}
    ...
    def get_input_value(noticia, campo):
        valor_atual = getattr(noticia, campo)
        if isinstance(valor_atual, (datetime, date)):
            return st.date_input(campo, value=valor_atual)
        elif isinstance(valor_atual, int):
            return st.number_input(campo, value=valor_atual)
        else:
            return st.text_input(campo, value=valor_atual or '')
...
def render_status(txt_label, txt):
    ...
    bg_color = get_status_bg_color(txt)
...
def get_status_bg_color(txt):
    if '99-' in txt:
        return '#f8d7da'  # Vermelho
    elif '10-' in txt:
        return '#b2e6b2'  # Verde
    elif '15-' in txt or '07-' in txt:
        return '#fff3cd'  # Amarelo
    return '#fff8e1'  # Neutro
